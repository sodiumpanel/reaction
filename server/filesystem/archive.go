...
type Archive struct {
	...
	// CompProgress is the compression progress tracker for WebSocket events.
	CompProgress *CompressProgress
...
func (a *Archive) addToArchive(dirfd int, name, relative string, entry ufs.DirEntry) error {
	...
	if a.CompProgress != nil {
		a.CompProgress.FileCompleted(header.Size)
	}
