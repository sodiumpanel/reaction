...
func postServerCompressFiles(c *gin.Context) {
	...
	// Get files info for progress tracking
	filesInfo, infoErr := s.Filesystem().GetFilesInfo(data.RootPath, data.Files)
	if infoErr != nil {
		log.WithField("error", infoErr).Debug("could not get files info for progress tracking, proceeding without progress")
	}

	// Create progress tracker
	progress := filesystem.NewCompressProgress(func(p filesystem.CompressProgressData) {
		s.Events().Publish(server.CompressProgressEvent, p)
	})

	// Set totals if we have them
	if infoErr == nil {
		progress.SetTotals(filesInfo.TotalFiles, filesInfo.TotalBytes)
	}

	// Emit initial progress
	progress.ForceEmit()

	f, compressErr := s.Filesystem().CompressFilesWithProgress(context.Background(), data.RootPath, data.Files, progress)

	// Emit completion event
	s.Events().Publish(server.CompressCompletedEvent, progress.CompletedData(compressErr))

	if compressErr != nil {
		middleware.CaptureAndAbort(c, compressErr)
		return
	}
...
func postServerDecompressFiles(c *gin.Context) {
	...
	archiveInfo, infoErr := s.Filesystem().GetArchiveInfo(context.Background(), data.RootPath, data.File)
	if infoErr != nil {
		lg.WithField("error", infoErr).Debug("could not get archive info for progress tracking, proceeding without progress")
	}

	// Create progress tracker
	progress := filesystem.NewDecompressProgress(data.File, func(p filesystem.DecompressProgressData) {
		s.Events().Publish(server.DecompressProgressEvent, p)
	})

	// Set totals if we have them
	if infoErr == nil {
		progress.SetTotals(archiveInfo.TotalFiles, archiveInfo.TotalBytes)
	}
	...
	decompressErr := s.Filesystem().DecompressFileWithProgress(context.Background(), data.RootPath, data.File, progress)

	// Emit completion event
	s.Events().Publish(server.DecompressCompletedEvent, progress.CompletedData(decompressErr))

	if decompressErr != nil {
		// If the file is busy for some reason just return a nicer error to the user since there is not
		// much we specifically can do. They'll need to stop the running server process in order to overwrite
		// a file like this.
		if strings.Contains(decompressErr.Error(), "text file busy") {
			lg.WithField("error", errors.WithStackIf(decompressErr)).Warn("failed to decompress file: text file busy")
			c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{
				"error": "One or more files this archive is attempting to overwrite are currently in use by another process. Please try again.",
			})
			return
		}
		middleware.CaptureAndAbort(c, decompressErr)
		return
	}
	c.Status(http.StatusNoContent)
}
